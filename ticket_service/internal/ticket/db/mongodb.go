...
// FindById find ticket by ticketID
func (d *db) FindById(ctx context.Context, id string) (ticket ticket.Ticket, err error) {
	...
	if result.Err() != nil {
		...
		return ticket, fmt.Errorf("failed to find ticket by id: %s due to error: %v", id, result.Err())
	}
	if err = result.Decode(&ticket); err != nil {
		return ticket, fmt.Errorf("failed to decode ticket(id:%s) from DB due to error: %v", id, err)
	}
	return ticket, nil
}
...
func (d *db) Update(ctx context.Context, ticket ticket.Ticket) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert ticket ID to ObjectID. ID=%v", ticket.ID)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to marshal ticket due to: %v", err)
	}

	var updateUserObj bson.M
	err = bson.Unmarshal(userBytes, &updateUserObj)
	if err != nil {
		return fmt.Errorf("failed to unmarshal ticket bytes due to: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to execute update ticket query due to: %v", err)
	}
...
func (d *db) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert ticket ID to ObjectID. ID=%v", id)
	}
