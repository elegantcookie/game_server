...
import (
	...
	"prize_service/internal/prize/db"
	"prize_service/pkg/client/mongodb"
	"prize_service/pkg/logging"
	"prize_service/pkg/metrics"
	"time"
)
...
func NewApp(cfg *config.Config, logger *logging.Logger) (App, error) {
	...
	service, err := prize.NewService(storage, *logger)
	if err != nil {
		panic(err)
	}

	usersHandler := prize.Handler{
