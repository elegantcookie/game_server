...
func (d *db) FindById(ctx context.Context, id string) (prize prize.Prize, err error) {
	...
	if result.Err() != nil {
		...
		return prize, fmt.Errorf("failed to find lobby by id: %s due to error: %v", id, result.Err())
	}
	if err = result.Decode(&prize); err != nil {
		return prize, fmt.Errorf("failed to decode lobby(id:%s) from DB due to error: %v", id, err)
	}
	return prize, nil
}
...
func (d *db) Update(ctx context.Context, prize prize.Prize) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert lobby ID to ObjectID. ID=%v", prize.ID)
	}
